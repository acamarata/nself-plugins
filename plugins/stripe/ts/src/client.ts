...
export class StripeClient {
  ...
  private mapSubscriptionSchedule(schedule: Stripe.SubscriptionSchedule): StripeSubscriptionScheduleRecord {
    return {
      ...
      released_subscription: typeof schedule.released_subscription === 'string' ? schedule.released_subscription : schedule.released_subscription?.id ?? null,
  ...
  private mapInvoiceItem(item: Stripe.InvoiceItem): StripeInvoiceItemRecord {
    return {
      ...
      discounts: (item.discounts ?? []).map(d => typeof d === 'string' ? d : d.id),
  ...
  private mapCharge(charge: Stripe.Charge): StripeChargeRecord {
    return {
      ...
      payment_method: typeof charge.payment_method === 'string' ? charge.payment_method : charge.payment_method?.id ?? null,
      ...
      source_id: typeof charge.source === 'string' ? charge.source : charge.source?.id ?? null,
  ...
  private mapRefund(refund: Stripe.Refund): StripeRefundRecord {
    return {
      ...
      failure_balance_transaction_id: typeof refund.failure_balance_transaction === 'string' ? refund.failure_balance_transaction : refund.failure_balance_transaction?.id ?? null,
  ...
  private mapDispute(dispute: Stripe.Dispute): StripeDisputeRecord {
    return {
      ...
      balance_transaction_id: typeof dispute.balance_transaction === 'string' ? dispute.balance_transaction : dispute.balance_transaction?.id ?? null,
  ...
  private mapSetupIntent(si: Stripe.SetupIntent): StripeSetupIntentRecord {
    return {
      ...
      automatic_payment_methods: si.automatic_payment_methods,
  ...
  private mapCheckoutSession(session: Stripe.Checkout.Session): StripeCheckoutSessionRecord {
    return {
      ...
      tax_id_collection: session.tax_id_collection,
