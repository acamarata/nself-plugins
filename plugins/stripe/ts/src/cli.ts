...
program
  ...
  .action(async (options) => {
    try {
      ...
      const client = new StripeClient(config.stripeApiKey, config.stripeApiVersion);
...
program
  ...
  .action(async () => {
    try {
      ...
      console.log('====================');
      console.log(`Mode: ${isTestMode(config.stripeApiKey) ? 'TEST' : 'LIVE'}`);
      console.log('\nSynced Records:');
    ...
    try {
      ...
      switch (action) {
        case 'list': {
          ...
          customers.forEach(c => {
            console.log(`${c.id} | ${c.email ?? 'N/A'} | ${c.name ?? 'N/A'}`);
          });
        ...
        case 'sync': {
          ...
          const syncService = new StripeSyncService(client, db);
          if (id) {
            await syncService.syncSingleResource('customer', id);
            logger.success(`Synced customer ${id}`);
          } else {
            const customers = await client.listAllCustomers();
            await db.upsertCustomers(customers);
            logger.success(`Synced ${customers.length} customers`);
          }
          break;
        }
        default:
      ...
      switch (action) {
        case 'list': {
          ...
          subscriptions.forEach(s => {
            console.log(`${s.id} | ${s.customer_id} | ${s.status} | ${s.current_period_end.toISOString().split('T')[0]}`);
          });
      ...
      switch (action) {
        case 'list': {
          ...
          invoices.forEach(i => {
            ...
            console.log(`${i.id} | ${i.customer_email ?? 'N/A'} | ${i.status} | ${i.currency.toUpperCase()} ${amount} | ${i.created_at.toISOString().split('T')[0]}`);
          });
    ...
    try {
      ...
      switch (action) {
        case 'list': {
          ...
          console.log('-'.repeat(80));
          products.forEach(p => {
            console.log(`${p.id} | ${p.name} | ${p.active ? 'Active' : 'Inactive'}`);
          });
      ...
      switch (action) {
        case 'list': {
          ...
          prices.forEach(p => {
            ...
            console.log(`${p.id} | ${p.product_id} | ${p.currency.toUpperCase()} ${amount} | ${p.type} | ${p.active ? 'Active' : 'Inactive'}`);
          });
