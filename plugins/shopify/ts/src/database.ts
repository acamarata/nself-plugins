...
export class ShopifyDatabase {
  ...
  async initializeSchema(): Promise<void> {
    ...
    await this.db.executeSqlFile(`
      ...
      CREATE TABLE IF NOT EXISTS shopify_locations (
        id BIGINT PRIMARY KEY,
        shop_id BIGINT REFERENCES shopify_shops(id) ON DELETE SET NULL,
        name VARCHAR(255) NOT NULL,
        address1 TEXT,
        address2 TEXT,
        city VARCHAR(255),
    ...
    await this.db.executeSqlFile(`
      ...
      CREATE TABLE IF NOT EXISTS shopify_discount_codes (
        id BIGINT PRIMARY KEY,
        price_rule_id BIGINT REFERENCES shopify_price_rules(id) ON DELETE CASCADE,
    ...
    await this.db.executeSqlFile(`
      ...
      FROM shopify_orders o
      LEFT JOIN shopify_fulfillments f ON o.id = f.order_id
      ...
      LEFT JOIN shopify_discount_codes dc ON pr.id = dc.price_rule_id
  ...
  async upsertShop(shop: ShopifyShopRecord): Promise<void> {
    await this.db.execute(
      `INSERT INTO shopify_shops (
        id, name, email, domain, myshopify_domain, shop_owner, phone,
      ...
      [
        shop.id, shop.name, shop.email, shop.domain, shop.myshopify_domain,
  ...
  async getShop(): Promise<ShopifyShopRecord | null> {
    return this.db.queryOne<ShopifyShopRecord>('SELECT * FROM shopify_shops LIMIT 1');
  }
  ...
  async upsertLocation(location: ShopifyLocationRecord): Promise<void> {
    await this.db.execute(
      `INSERT INTO shopify_locations (
        id, shop_id, name, address1, address2, city, province, province_code,
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        updated_at = EXCLUDED.updated_at, synced_at = NOW()`,
      [
        location.id, location.shop_id, location.name, location.address1, location.address2,
  ...
  async listLocations(): Promise<ShopifyLocationRecord[]> {
    const result = await this.db.query<ShopifyLocationRecord>(
      'SELECT * FROM shopify_locations WHERE active = true ORDER BY name'
    );
    return result.rows;
  }

  async countLocations(): Promise<number> {
    return this.db.count('shopify_locations');
  }
  ...
  async upsertProduct(product: ShopifyProductRecord): Promise<void> {
    await this.db.execute(
      `INSERT INTO shopify_products (
        id, shop_id, title, body_html, vendor, product_type, handle, status,
      ...
      [
        product.id, product.shop_id, product.title, product.body_html,
  ...
  async listProducts(limit = 100, offset = 0): Promise<ShopifyProductRecord[]> {
    const result = await this.db.query<ShopifyProductRecord>(
      'SELECT * FROM shopify_products ORDER BY updated_at DESC LIMIT $1 OFFSET $2',
      [limit, offset]
    );
    return result.rows;
  }
  ...
  async countProducts(): Promise<number> {
    return this.db.count('shopify_products');
  }

  async getProduct(id: number): Promise<ShopifyProductRecord | null> {
    return this.db.queryOne<ShopifyProductRecord>(
      'SELECT * FROM shopify_products WHERE id = $1',
      [id]
    );
  }

  async deleteProduct(id: number): Promise<void> {
    await this.db.execute('DELETE FROM shopify_products WHERE id = $1', [id]);
  }
  ...
  async upsertVariant(variant: ShopifyVariantRecord): Promise<void> {
    await this.db.execute(
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        updated_at = EXCLUDED.updated_at, synced_at = NOW()`,
      [
        variant.id, variant.product_id, variant.title, variant.price,
  ...
  async countVariants(): Promise<number> {
    return this.db.count('shopify_variants');
  }
  ...
  async getProductVariants(productId: number): Promise<ShopifyVariantRecord[]> {
    const result = await this.db.query<ShopifyVariantRecord>(
      'SELECT * FROM shopify_variants WHERE product_id = $1 ORDER BY position',
      [productId]
    );
    return result.rows;
  }
  ...
  async upsertCollection(collection: ShopifyCollectionRecord): Promise<void> {
    await this.db.execute(
      `INSERT INTO shopify_collections (
        id, shop_id, title, body_html, handle, collection_type, sort_order,
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        updated_at = EXCLUDED.updated_at, synced_at = NOW()`,
      [
        collection.id, collection.shop_id, collection.title, collection.body_html,
  ...
  async countCollections(): Promise<number> {
    return this.db.count('shopify_collections');
  }
  ...
  async listCollections(limit = 100, offset = 0): Promise<ShopifyCollectionRecord[]> {
    const result = await this.db.query<ShopifyCollectionRecord>(
      'SELECT * FROM shopify_collections ORDER BY title LIMIT $1 OFFSET $2',
      [limit, offset]
    );
    return result.rows;
  }
  ...
  async deleteCollection(id: number): Promise<void> {
    await this.db.execute('DELETE FROM shopify_collections WHERE id = $1', [id]);
  }
  ...
  async upsertCustomer(customer: ShopifyCustomerRecord): Promise<void> {
    await this.db.execute(
      `INSERT INTO shopify_customers (
        id, shop_id, email, first_name, last_name, phone, verified_email,
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        updated_at = EXCLUDED.updated_at, synced_at = NOW()`,
      [
        customer.id, customer.shop_id, customer.email, customer.first_name,
  ...
  async listCustomers(limit = 100, offset = 0): Promise<ShopifyCustomerRecord[]> {
    const result = await this.db.query<ShopifyCustomerRecord>(
      'SELECT * FROM shopify_customers ORDER BY updated_at DESC LIMIT $1 OFFSET $2',
      [limit, offset]
    );
    return result.rows;
  }
  ...
  async countCustomers(): Promise<number> {
    return this.db.count('shopify_customers');
  }

  async getCustomer(id: number): Promise<ShopifyCustomerRecord | null> {
    return this.db.queryOne<ShopifyCustomerRecord>(
      'SELECT * FROM shopify_customers WHERE id = $1',
      [id]
    );
  }

  async deleteCustomer(id: number): Promise<void> {
    await this.db.execute('DELETE FROM shopify_customers WHERE id = $1', [id]);
  }
  ...
  async upsertOrder(order: ShopifyOrderRecord): Promise<void> {
    await this.db.execute(
      `INSERT INTO shopify_orders (
        id, shop_id, order_number, name, email, phone, customer_id,
      ...
      [
        order.id, order.shop_id, order.order_number, order.name, order.email,
  ...
  async listOrders(status?: string, limit = 100, offset = 0): Promise<ShopifyOrderRecord[]> {
    if (status) {
      const result = await this.db.query<ShopifyOrderRecord>(
        'SELECT * FROM shopify_orders WHERE financial_status = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3',
        [status, limit, offset]
      );
      return result.rows;
    }
    const result = await this.db.query<ShopifyOrderRecord>(
      'SELECT * FROM shopify_orders ORDER BY created_at DESC LIMIT $1 OFFSET $2',
      [limit, offset]
    );
    return result.rows;
  }
  ...
  async countOrders(status?: string): Promise<number> {
    if (status) return this.db.count('shopify_orders', 'financial_status = $1', [status]);
    return this.db.count('shopify_orders');
  }

  async deleteOrder(id: number): Promise<void> {
    await this.db.execute('DELETE FROM shopify_orders WHERE id = $1', [id]);
  }
  ...
  async upsertOrderItem(item: ShopifyOrderItemRecord): Promise<void> {
    await this.db.execute(
      `INSERT INTO shopify_order_items (
        id, order_id, product_id, variant_id, title, variant_title, sku,
      ...
      [
        item.id, item.order_id, item.product_id, item.variant_id, item.title,
  ...
  async upsertFulfillment(fulfillment: ShopifyFulfillmentRecord): Promise<void> {
    await this.db.execute(
      `INSERT INTO shopify_fulfillments (
        id, order_id, location_id, status, tracking_company, tracking_number,
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        updated_at = EXCLUDED.updated_at, synced_at = NOW()`,
      [
        fulfillment.id, fulfillment.order_id, fulfillment.location_id, fulfillment.status,
  ...
  async countFulfillments(): Promise<number> {
    return this.db.count('shopify_fulfillments');
  }
  ...
  async upsertTransaction(transaction: ShopifyTransactionRecord): Promise<void> {
    await this.db.execute(
      `INSERT INTO shopify_transactions (
        id, order_id, parent_id, kind, gateway, status, message, amount, currency,
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        synced_at = NOW()`,
      [
        transaction.id, transaction.order_id, transaction.parent_id, transaction.kind,
  ...
  async countTransactions(): Promise<number> {
    return this.db.count('shopify_transactions');
  }
  ...
  async upsertRefund(refund: ShopifyRefundRecord): Promise<void> {
    await this.db.execute(
      `INSERT INTO shopify_refunds (
        id, order_id, note, restock, user_id, refund_line_items, transactions,
      ...
      [
        refund.id, refund.order_id, refund.note, refund.restock, refund.user_id,
  ...
  async countRefunds(): Promise<number> {
    return this.db.count('shopify_refunds');
  }
  ...
  async upsertDraftOrder(draftOrder: ShopifyDraftOrderRecord): Promise<void> {
    await this.db.execute(
      `INSERT INTO shopify_draft_orders (
        id, shop_id, order_id, name, email, customer_id, status, currency,
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        updated_at = EXCLUDED.updated_at, synced_at = NOW()`,
      [
        draftOrder.id, draftOrder.shop_id, draftOrder.order_id, draftOrder.name,
  ...
  async countDraftOrders(): Promise<number> {
    return this.db.count('shopify_draft_orders');
  }

  async deleteDraftOrder(id: number): Promise<void> {
    await this.db.execute('DELETE FROM shopify_draft_orders WHERE id = $1', [id]);
  }
  ...
  async upsertInventoryItem(item: ShopifyInventoryItemRecord): Promise<void> {
    await this.db.execute(
      `INSERT INTO shopify_inventory_items (
        id, sku, cost, country_code_of_origin, country_harmonized_system_codes,
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        updated_at = EXCLUDED.updated_at, synced_at = NOW()`,
      [
        item.id, item.sku, item.cost, item.country_code_of_origin,
  ...
  async countInventoryItems(): Promise<number> {
    return this.db.count('shopify_inventory_items');
  }
  ...
  async upsertInventory(inventory: ShopifyInventoryRecord): Promise<void> {
    await this.db.execute(
      `INSERT INTO shopify_inventory (
        inventory_item_id, location_id, variant_id, available, incoming,
      ...
      [
        inventory.inventory_item_id, inventory.location_id, inventory.variant_id,
  ...
  async countInventory(): Promise<number> {
    return this.db.count('shopify_inventory');
  }
  ...
  async listInventory(limit = 100, offset = 0): Promise<ShopifyInventoryRecord[]> {
    const result = await this.db.query<ShopifyInventoryRecord>(
      'SELECT * FROM shopify_inventory ORDER BY inventory_item_id LIMIT $1 OFFSET $2',
      [limit, offset]
    );
    return result.rows;
  }
  ...
  async upsertPriceRule(priceRule: ShopifyPriceRuleRecord): Promise<void> {
    await this.db.execute(
      `INSERT INTO shopify_price_rules (
        id, shop_id, title, target_type, target_selection, allocation_method,
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        updated_at = EXCLUDED.updated_at, synced_at = NOW()`,
      [
        priceRule.id, priceRule.shop_id, priceRule.title, priceRule.target_type,
  ...
  async countPriceRules(): Promise<number> {
    return this.db.count('shopify_price_rules');
  }

  async deletePriceRule(id: number): Promise<void> {
    await this.db.execute('DELETE FROM shopify_price_rules WHERE id = $1', [id]);
  }
  ...
  async upsertDiscountCode(discountCode: ShopifyDiscountCodeRecord): Promise<void> {
    await this.db.execute(
      `INSERT INTO shopify_discount_codes (
        id, price_rule_id, code, usage_count, admin_graphql_api_id,
        created_at, updated_at, synced_at
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        updated_at = EXCLUDED.updated_at, synced_at = NOW()`,
      [
        discountCode.id, discountCode.price_rule_id, discountCode.code,
  ...
  async countDiscountCodes(): Promise<number> {
    return this.db.count('shopify_discount_codes');
  }

  async deleteDiscountCode(id: number): Promise<void> {
    await this.db.execute('DELETE FROM shopify_discount_codes WHERE id = $1', [id]);
  }
  ...
  async upsertGiftCard(giftCard: ShopifyGiftCardRecord): Promise<void> {
    await this.db.execute(
      `INSERT INTO shopify_gift_cards (
        id, shop_id, balance, initial_value, currency, code, last_characters,
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        updated_at = EXCLUDED.updated_at, synced_at = NOW()`,
      [
        giftCard.id, giftCard.shop_id, giftCard.balance, giftCard.initial_value,
  ...
  async countGiftCards(): Promise<number> {
    return this.db.count('shopify_gift_cards');
  }

  async disableGiftCard(id: number): Promise<void> {
    await this.db.execute(
      'UPDATE shopify_gift_cards SET disabled_at = NOW() WHERE id = $1',
      [id]
    );
  }
  ...
  async upsertMetafield(metafield: ShopifyMetafieldRecord): Promise<void> {
    await this.db.execute(
      `INSERT INTO shopify_metafields (
        id, namespace, key, value, type, description, owner_id, owner_resource,
        admin_graphql_api_id, created_at, updated_at, synced_at
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        updated_at = EXCLUDED.updated_at, synced_at = NOW()`,
      [
        metafield.id, metafield.namespace, metafield.key, metafield.value,
  ...
  async countMetafields(): Promise<number> {
    return this.db.count('shopify_metafields');
  }

  async deleteMetafield(id: number): Promise<void> {
    await this.db.execute('DELETE FROM shopify_metafields WHERE id = $1', [id]);
  }
  ...
  async upsertCheckout(checkout: ShopifyCheckoutRecord): Promise<void> {
    await this.db.execute(
      `INSERT INTO shopify_checkouts (
        id, token, cart_token, shop_id, email, customer_id, customer_locale,
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        updated_at = EXCLUDED.updated_at, synced_at = NOW()`,
      [
        checkout.id, checkout.token, checkout.cart_token, checkout.shop_id,
  ...
  async countCheckouts(): Promise<number> {
    return this.db.count('shopify_checkouts');
  }
  ...
  async insertWebhookEvent(event: ShopifyWebhookEventRecord): Promise<void> {
    await this.db.execute(
      `INSERT INTO shopify_webhook_events (
        id, topic, shop_id, shop_domain, data, processed, processed_at, error, received_at
      ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW())
      ON CONFLICT (id) DO UPDATE SET
        processed = EXCLUDED.processed, processed_at = EXCLUDED.processed_at, error = EXCLUDED.error`,
      [
        event.id, event.topic, event.shop_id, event.shop_domain,
  ...
  async listWebhookEvents(topic?: string, limit = 50): Promise<ShopifyWebhookEventRecord[]> {
    if (topic) {
      const result = await this.db.query<ShopifyWebhookEventRecord>(
        'SELECT * FROM shopify_webhook_events WHERE topic = $1 ORDER BY received_at DESC LIMIT $2',
        [topic, limit]
      );
      return result.rows;
    }
    const result = await this.db.query<ShopifyWebhookEventRecord>(
      'SELECT * FROM shopify_webhook_events ORDER BY received_at DESC LIMIT $1',
      [limit]
    );
    return result.rows;
  }
